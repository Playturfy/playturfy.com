...
// Live Counter - Marriage Date: 25/11/2025
function initLiveCounter() {
    const marriageDate = new Date('2025-11-25T00:00:00');
...
function initButtons() {
    ...
    yesBtn.addEventListener('click', () => {
        ...
        noBtn.style.cssText = 'display: none !important; visibility: hidden !important; opacity: 0 !important;';
    ...
    noBtn.addEventListener('click', (e) => {
        ...
        // Move button immediately after text change
        moveNoButton(noBtn);
    ...
    // Also move on hover for desktop - DISABLED to prevent conflict with click
...
// Move No Button to Random Position (around the Yes button)
function moveNoButton(button) {
    // Add moving class for higher z-index
    button.classList.add('moving');
    
    // SAVE the button content BEFORE moving
    const buttonContent = button.innerHTML;
    
    // MOVE BUTTON TO BODY to escape any parent container constraints
    if (button.parentElement.id !== 'body') {
    ...
    // Get actual button dimensions after text change
    const buttonWidth = button.offsetWidth || 200;
    const buttonHeight = button.offsetHeight || 60;
    
    // Try to find a position around Yes button that stays on screen
    let randomX, randomY;
    let attempts = 0;
    
    do {
        // Define radius around Yes button
        const minRadius = 80;
        const maxRadius = 150;
        ...
        // Calculate position
        randomX = yesCenterX + Math.cos(angle) * distance - buttonWidth / 2;
        randomY = yesCenterY + Math.sin(angle) * distance - buttonHeight / 2;
        
        // Check if within viewport
        const padding = 20;
        if (randomX >= padding && randomX + buttonWidth <= viewportWidth - padding &&
            randomY >= padding && randomY + buttonHeight <= viewportHeight - padding) {
    ...
    const padding = 20;
    const maxAllowedX = viewportWidth - buttonWidth - padding;
    const maxAllowedY = viewportHeight - buttonHeight - padding;
    
    randomX = Math.max(padding, Math.min(randomX, maxAllowedX));
    randomY = Math.max(padding, Math.min(randomY, maxAllowedY));
    ...
    // RESTORE the button content AFTER applying styles
